...
_load() {
	local _dir="/etc/v2ray/321boy/v2ray/src/"
	. "${_dir}$@"
}
_sys_timezone() {
	IS_OPENVZ=
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			...
			echo
			echo -e " 请确保你有完整的上传 smx4927 的 V2Ray 一键安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
		mkdir -p /etc/v2ray/321boy/v2ray
		cp -rf $(pwd)/* /etc/v2ray/321boy/v2ray
	else
		pushd /tmp
		git clone https://github.com/321boy/v2ray -b "$_gitbranch" /etc/v2ray/321boy/v2ray --depth=1
		popd

	fi

	if [[ ! -d /etc/v2ray/321boy/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/321boy/v2ray/config/backup.conf $backup
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/321boy/v2ray ]]; then
		...
		备注...仅支持卸载使用我 (smx4927) 提供的 V2Ray 一键安装脚本
		" && exit 1
	fi
...
while :; do
	echo
	echo "........... V2Ray 一键安装脚本 & 管理脚本 by 华人老王，原作者233boy .........."
	echo
	echo "帮助说明: 微信smx4927"
	echo
	echo "搭建教程: 微信smx4927"
	echo "免责声明: 仅供测试学习不得用于生产环境，请于下载后24小时之内删除"
	echo
	echo " 1. 安装"
	echo
	echo " 2. 卸载"
